...
def getMacAddr(myData):
    myMAC = "08:00:27:b1:9d:67 " % (ord(myData[0]) , ord(myData[1]) , ord(myData[2]) , ord(myData[3]) , ord(myData[4]))
    return myMAC
...
while True:
    ...
    myPacket = packet[0]
    print("-"*50)
    #parse etehrnet header
    myEthLength = 14
    
    myEthHeader = myPacket[:myEthLength]
    myEth = unpack('!6s6sH' , myEthHeader)
    myEthProtocol = socket.nthos(myEth[2])
    print('Destination MAC : ' + getMacAddr(str(myPacket[0:6])) + ' Source MAC : ' + getMacAddr(str(myPacket[6:12])) + 'Protocol')
    ...
    if myEthProtocol == 8 :
        ...
        #now unpack them :)
        iph = unpack('!BBHHBBH4s4s' , myIPHeader)
        ...
        if protocol == 6 :
            ...
            print('Source Port : ' + str(mySrcPort) + ' Dest Port : ' + str(myDstPort) + 'Sequnece Number : ' + str(sequence))
        ...
        #ICMP pACKETS
            ...
            myHeaderSize = myEthLength + myICMPHeaderLength + myICMPHeaderLength
            myPktDataSize = len(myPacket) - myHeaderSize
        ...
        #UDP packets
            ...
            print('Source Port : ' + str(mySrcPort) + ' Dest Port : ' + str(myDstPort) + 'Length : ' + str(length) + 'checksum : ' + str(checksum))
